...
void Camera::GetFrustumPlanes(Float4 outPlanes[6]) const
{
    DirectX::BoundingFrustum frustum(projection.ToSIMD());
    frustum.Transform(frustum, 1.0f, orientation.ToSIMD(), position.ToSIMD());

    DirectX::XMVECTOR planes[6];
    frustum.GetPlanes(&planes[0], &planes[1], &planes[2], &planes[3], &planes[4], &planes[5]);
    for(uint32 i = 0; i < 6; ++i)
        outPlanes[i] = Float4(planes[i]) * Float4(-1.0f, -1.0f, -1.0f, -1.0f);
}
