...
// Can't get includes to work so far with SHADERed :(
...
void main()
{
   SH_L1_RGB sh = SH_ProjectOntoL1_RGB(normalize(vec3(1.0f, 1.0f, 1.0f)), vec3(0.5f, 0.75f, 1.0f) * 2);
   float sinT = sin(time);
   float cosT = cos(time);
   mat3x3 rot;
   rot[0] = vec3(cosT, 0.0f, sinT);
   rot[1] = vec3(0.0f, 1.0f, 0.0f);
   rot[2] = normalize(cross(rot[0], rot[1]));
   sh = SH_Rotate(rot, sh);
   vec3 irradiance = SH_CalculateIrradiance(sh, normalWS);
   outColor = vec4(irradiance / vec3(M_PI), 1.0f);
}